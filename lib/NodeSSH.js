...
function SSHClient(options) {
  ...
  SSHClientSession.overBuffer = '';
}

SSHClient.prototype.connect = function connect() {
  if (SSHClientSession.connected === true || SSHClientSession.child) {
    //Log
    const errMsg = 'Ssh session has already been established. Disposing session and creating a new one';
    util.debug(errMsg);
    SSHClientSession.close(null); // Close
    SSHClientSession.connect(); // Reconnect
    return;
  }
  // Create Child
...
SSHClient.prototype.exec = function exec(cmd, callback) {
  ...
  const f = function(data, lastChunk) {
    if (lastChunk) {
      SSHClientSession.removeListener('output', f);
    }
...
SSHClient.prototype.resumeOutput = function resumeOutput() {
  if (SSHClientSession.connected === true && SSHClientSession.child && SSHClientSession.child.stdout &&
    SSHClientSession.paused === true) {
    if (SSHClientSession.overBuffer.length > 0) { SSHClientSession.read(''); }
...
SSHClient.prototype.read = function read(data) {
  ...
  if (SSHClientSession.connected === true) {
    ...
    if (SSHClientSession.overBuffer.length > 0) {
      data = SSHClientSession.overBuffer + data;
      SSHClientSession.overBuffer = '';
    }

    console.log('NodeSSH debug - pattern before checking against regex : ' + data)

    let isLast = false;
    if (data.match(/[#$]\s$/)) {
      console.log('NodeSSH debug - Pattern match with regEx');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (str) { //Valid ?
        if (SSHClientSession.paused === true) { //Check if has paused
          SSHClientSession.overBuffer = SSHClientSession.overBuffer + str + '\n';
...
SSHClient.prototype.close = function close(errorString) {
  if (SSHClientSession.child && SSHClientSession.child.kill) {
    SSHClientSession.child.kill();
  }
