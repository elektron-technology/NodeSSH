...
function SSHClient(options) {	SSHClientSession = this;
	...
	//Helpers
	SSHClientSession.buffer = "";
	SSHClientSession.connected = false;	SSHClientSession.child = null;
};


SSHClient.prototype.connect = function connect() {
	if (SSHClientSession.connected == true || SSHClientSession.child || (SSHClientSession.buffer && SSHClientSession.buffer.length > 0)) {
		//Log
		var errMsg = "SSHClient Session alerady have SSH child stabilished! Disconnecting and opening a new SSH session :)";
		util.debug(errMsg);
		SSHClientSession.close(); //close
		SSHClientSession.connect(); //recall connect
		return;
	}
	//Create Child
	//console.log([__dirname+'/login.exp', SSHClientSession.host, SSHClientSession.username, SSHClientSession.password, SSHClientSession.port, SSHClientSession.cert]);
...
//	SSHClientSession.child.stdin.on("data",function (data) { 
//		consolo.log('stderr',data.toString());
//	});
	//Child exit
	SSHClientSession.child.on("exit",function () {
		return SSHClientSession.close();
	});
};
...
SSHClient.prototype.read = function read(data) {
	...
	if(!data) { return; }
		...
		if (SSHClientSession.connected == true) {
			SSHClientSession.buffer = SSHClientSession.buffer + data;
			if(SSHClientSession.buffer.match(/[#|$]\s$/)) {
				var match = SSHClientSession.buffer.match(/\r\n\s?((?:[\n\r]|.)*)\r\n.*[#|$]\s$/);
				SSHClientSession.emit('output',match?match[1]:'', match);
				SSHClientSession.buffer = '';
			} else if (SSHClientSession.buffer && SSHClientSession.buffer.length == data.length) {  
				var lines = SSHClientSession.buffer.split("\n");
				var str = lines[0];
				if (str) { 
					str = str.replace("\r","");
					SSHClientSession.emit('input',str);
				}
			}
		}//Connection errors
			...
			console.error("->",data);
			SSHClientSession.emit('error',SSHClientSession.host, data);
			SSHClientSession.buffer = "";
			return SSHClientSession.close();
...
SSHClient.prototype.close = function close(){
	...
	if (SSHClientSession.child) {
		...
		SSHClientSession.child = null ;	
		//Emit close
		SSHClientSession.emit('closed',SSHClientSession.host);
	}
};